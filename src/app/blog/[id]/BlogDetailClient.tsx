...
import { BlogDetail } from "@/types/database";
import ShareModal from "@/components/shared/ShareModal";

export function BlogDetailView({ post }: { post: BlogDetail | null }) {
      ...
      <main className="relative bg-[#12130F] min-h-screen pb-20 md:pb-0" dir={isAr ? "rtl" : "ltr"}>
        ...
        <div className="max-w-4xl mx-auto pt-44 px-6 text-center py-20">
          ...
          <p className="font-sans text-sm text-[#C5BCAD] mb-8">
            {isAr ? "عذراً، لم يتم العثور على المقال المطلوب." : "The requested article could not be found."}
          </p>
          <Link
            href="/blog"
  ...
  const quote = isAr ? (post.quoteAr || post.quoteEn) : (post.quoteEn || post.quoteAr);
    ...
    <main className="relative bg-[#12130F] min-h-screen pb-20 md:pb-0" dir={isAr ? "rtl" : "ltr"}>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className={`flex items-center justify-between gap-4 mb-6 flex-wrap ${isAr ? "flex-row-reverse" : ""}`}>
            <div className={`flex items-center gap-2 font-mono text-[10px] tracking-[0.2em] uppercase text-[#8C8477] flex-wrap ${isAr ? "flex-row-reverse text-right" : ""}`}>
              ...
              <span>/</span>
              <span className="text-[#E8DFCE]">{isAr ? (post.categoryAr || post.categoryEn) : (post.categoryEn || post.categoryAr)}</span>
            </div>
          ...
          {/* Category Badge */}
          <div
            className={`inline-flex items-center gap-2 px-3.5 py-1 mb-6 border rounded-full ${isAr ? "flex-row-reverse" : ""}`}
          ...
          >
            ...
            <span className="font-mono text-[10px] tracking-[0.25em] uppercase font-semibold" style={{ color: post.accent }}>
              {isAr ? (post.categoryAr || post.categoryEn) : (post.categoryEn || post.categoryAr)}
            </span>
          </div>
          ...
          <h1 className={`font-display text-3xl sm:text-5xl lg:text-6xl text-[#E8DFCE] font-normal leading-[1.12] tracking-[-0.02em] mb-6 ${isAr ? "text-right" : ""}`}>
            {isAr ? (post.titleAr || post.titleEn) : (post.titleEn || post.titleAr)}
          </h1>
          ...
          <div className={`flex flex-wrap items-center gap-4 text-xs font-mono text-[#8C8477] pt-4 border-t border-white/5 ${isAr ? "flex-row-reverse text-right" : ""}`}>
            <span className="text-[#E8DFCE] font-medium">
              {isAr ? (post.authorAr || post.authorEn) : (post.authorEn || post.authorAr)}
            </span>
            <span>•</span>
            <span>{isAr ? (post.dateAr || post.dateEn) : (post.dateEn || post.dateAr)}</span>
            <span>•</span>
            <span>{isAr ? (post.readTimeAr || post.readTimeEn) : (post.readTimeEn || post.readTimeAr)}</span>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 px-6 sm:px-10 lg:px-20">
        <div className="max-w-4xl mx-auto space-y-12">
          ...
          {post.statBox && post.statBox.length > 0 && (
              ...
              {post.statBox.map((st, i) => (
                <div key={i} className="p-6 border border-[#B8873B]/20 bg-[#12130F] text-center">
                  ...
                  <div className="font-mono text-[9px] tracking-[0.2em] uppercase text-[#8C8477]">
                    {isAr ? (st.labelAr || st.labelEn) : (st.labelEn || st.labelAr)}
                  </div>
                </div>
              ))}
            </div>
          ...
          {quote && (
            ...
            >
              ...
              <footer className="font-mono text-[10px] tracking-[0.25em] uppercase text-[#B8873B] not-italic mt-4">
                — {isAr ? "استشارات أصاهيب العقارية" : "Asaheeb Investment Advisory"}
              </footer>
            </blockquote>
          )}
      ...
      <ShareModal
        ...
        title={isAr ? (post.titleAr || post.titleEn) : (post.titleEn || post.titleAr)}
        text={isAr ? `مقال من مدونة أصاهيب العقارية: ${post.titleAr || post.titleEn}` : `Article from Asaheeb Real Estate Blog: ${post.titleEn || post.titleAr}`}
...
export default function DynamicBlogDetailClient({ post }: { post: BlogDetail | null }) {
  return (
    <LanguageProvider>
